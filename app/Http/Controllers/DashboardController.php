...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $customers = Company::where('parent_company_id', '=', $company_id)->where('id', '!=', $company_id)->count();
        $drivers = User::where('company_id', '=', $company_id)->where('usertype', '=', 'driver')->count();
        $shipments = Txn::where('company_id', '=', $company_id)->where(DB::raw('DATE_FORMAT(txn_date, "%Y-%m")'), '=', $curr_mon)->where('parcel_status_id','!=','6')->count();
        $percompany = Txn::where('company_id', '=', $company_id)->where(DB::raw('DATE_FORMAT(txn_date, "%Y-%m")'), '=', $curr_mon)->where('parcel_status_id','!=','6')->select('sender_company_id', DB::raw('count(id) as shipment_count'))->groupBy('sender_company_id')->orderBy('shipment_count', 'desc')->get();
        $perrider = Txn::where('company_id', '=', $company_id)->where(DB::raw('DATE_FORMAT(txn_date, "%Y-%m")'), '=', $curr_mon)->where('parcel_status_id','!=','6')->select('driver_id', DB::raw('count(id) as shipment_count'))->groupBy('driver_id')->orderBy('shipment_count', 'desc')->get();
        return view('dashboard.index', ['customers' => $customers, 'drivers' => $drivers, 'shipments' => $shipments, 'percompany' => $percompany, 'perrider' => $perrider]);
